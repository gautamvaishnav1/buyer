  ...
  const RFQTableActions = (options: any) => {
    return (
      <div className='btnEditDelete'>
        <LinkButton
          styleName='blue'
          link={`/view-rfq/${options.id}`}
        ><GrView /></LinkButton>
        <LinkButton
          styleName='green'
          link={`edit-rfq/${options.id}`}
        ><LiaEdit /></LinkButton>
        <LinkButton
          styleName='red'
    ...
    <div className='product-mgmt'>
      ...
      <section className='table-section'>
        ...
        <section className='mt-2'>
          <DataTable value={filteredRFQs} paginator rows={5} rowsPerPageOptions={[5, 10, 25, 50]} tableStyle={{ minWidth: '50rem' }}
        paginatorTemplate="RowsPerPageDropdown FirstPageLink PrevPageLink CurrentPageReport NextPageLink LastPageLink">
            <Column header='#' body={countFunction} />
            <Column header='RFQ ID' body={rfqIdBody} />
            <Column header='Date' body={dateBody} />
            <Column header='Buyer' body={buyerBody} />
            <Column header='Category' body={categoryBody} />
            <Column header='Quantity' body={quantityBody} />
            <Column header='Budget' body={budgetBody} />
            <Column header='Status' body={statusBody} />
            <Column header='Actions' body={RFQTableActions} />
          </DataTable>
        </section>
      </section>
    </div>
